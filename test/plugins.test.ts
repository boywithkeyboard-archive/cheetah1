...
Deno.test('Plugins', async (t) => {
  await t.step('beforeParsing (global)', async () => {
    ...
    const result =
      await (await app.fetch(new Request('http://localhost:3000/test'))).text()
  ...
  await t.step('beforeParsing (prefix)', async () => {
    ...
    const result2 =
      await (await app.fetch(new Request('http://localhost:3000/prefix/test')))
        .text()
    assertEquals(result2, 'Hello World')

    const result3 = await (await app.fetch(
      new Request('http://localhost:3000/prefixinvalid/test'),
    )).text()
  ...
  await t.step('afterParsing (global)', async () => {
    ...
    const result2 =
      await (await app.fetch(new Request('http://localhost:3000/another_test')))
        .text()
    assertEquals(result2, 'Hello World')
  })
})
